...
def get_policies_for_users(path, merged_data):
    ...
    valid_user_list = [
        user.encode() for user in merged_data.keys() if is_valid_arn(user.encode())
    ]

    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(get_previous_policies, iam_client, path, user.decode())
...
def get_previous_policies(iam_client, path, current_user):
    ...
    try:
        ...
        if user_exists(iam_client, current_user):
            policies = iam_client.list_attached_user_policies(UserName=current_user)[
                "AttachedPolicies"
            ]
            inline_policies = iam_client.list_user_policies(UserName=current_user)[
                "PolicyNames"
            ]
            ...
            for policy_name in inline_policies:
                policy_doc = iam_client.get_user_policy(
                    UserName=current_user, PolicyName=policy_name
                )["PolicyDocument"]
                policy_dict["Statement"].extend(policy_doc["Statement"])
        else:
        ...
        outfileName = (
            outfileName.replace(":", "-").replace("/", "_").replace("\\", "__")
        )
        with open(os.path.join(path, outfileName), "w") as f:
            json.dump(policy_dict, f, indent=2)

        print(f"Fetched Present Policies for {current_user}")
