...
def generateLeastprivPolicies(user_arn, policy_output_dir, actions_list):
    ...
    policy = generate_least_privilege_policy(
        user_arn, actions_list, service_actions_cache, service_replace_map
    )
        ...
        for prefix, event, value in parser:
            if prefix == startKey:
                if startLoop == False:
                    ...
                    startLoop = False
                    if (
                        currentBlock.get("username") is not None
                        and currentBlock.get("arn") is not None
                    ):
                        userNameCB = currentBlock.get("username")
                        outerKey = f"{accountID}_{bucketName}_{date}_{region}"
                        crudConnection.push_back_nested_json(
                            outerKey, userNameCB, currentBlock
                        )
                    currentBlock = {}
            if startLoop:
                ...
                if (
                    prefix == "Records.item.userIdentity.arn"
                    and currentBlock.get("userIdentityType") == "IAMUser"
                ):
